...
function Invoke-Pip {
    ...
    # 自动添加清华镜像（如果用户没自己写 -i）
    if ($Args -notcontains "-i" -and ($Args -notmatch "https://")) {
        $Args += "-i"
        $Args += "https://pypi.tuna.tsinghua.edu.cn/simple"
    }

    Write-Host "Running: pip install $Args" -ForegroundColor Cyan
    ...
    if ($exitCode -ne 0) {
        ...
        Write-Host "`nPip Install Failed" -ForegroundColor Red
        ...
        foreach ($item in $output) {
            if ($item -is [System.Management.Automation.ErrorRecord]) {
                ...
                if ($msg -match "WinError 32" -or $msg -match "Permission denied") {
                    $isCritical = $true
                }
            }
            else {
                ...
                if ($item -match "WinError 32" -or $item -match "Permission denied") {
                    $isCritical = $true
                }
            }
        }
        ...
        if ($isCritical) {
            Write-Host "`nDetected critical file lock or permission issue." -ForegroundColor Yellow
            Write-Host "Try closing other processes or using --user option." -ForegroundColor Yellow
            throw [System.Exception]::new(($errorMessages -join "`n"))
        }
        else {
            Write-Warning "`nNon-critical errors found, continuing with script..."
        }
    }
    else {
        Write-Host "`nPip Install Success ✅" -ForegroundColor Green
    }
}
